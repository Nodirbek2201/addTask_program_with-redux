...
import {addKirim,editKirim} from "../Redux/Action/Action";

function KirimModal({modalVisible,toggleModal,addKirim,modalStatus,kirim_id,kirimlar,editKirim,foydalanuvchilar,kassalar}) {
    ...
    useEffect(()=>{
        if(kirim_id.id){
            kirimlar.forEach(item=>{
                if(item.id===kirim_id.id)
                    setInputVals({
                        id:kirim_id.id,
...
export default connect(({foydalanuvchiReducer,kassaReducer})=>({foydalanuvchilar:foydalanuvchiReducer.foydalanuvchilar,kassalar:kassaReducer.kassalar}),{addKirim,editKirim})(KirimModal);